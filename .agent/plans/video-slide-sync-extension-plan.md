...
## Goal
Add a generalized video slide capability, with YouTube as the first implementation target, that:
- lets deck authors declare one or more synchronized video slides without custom per-deck sync logic
...
- Instructor-local persisted student-audio preference is only a toolbar default for future sessions; it does not replace live per-session authority.
...
- navigation boundary changes alone do not change playback authority; they only affect whether student playback remains allowed, is cut off, or is rubber-banded by existing sync rules
...
- hosts and iframe runtimes should tolerate unrecognized video-specific commands/events for forward compatibility; unknown message types should be ignored rather than treated as fatal protocol errors
...
- if authority changes while local student playback is enabled, immediately reapply the new active instructor's current audio and local-control policy
...
- navigation boundary/storyboard changes do not by themselves take playback authority
...
- persisted student-audio preference is an instructor-toolbar startup default for future sessions only

Readiness aggregation contract:
- host must aggregate `youtubePlayerStatus` per connected student using the existing student/session IDs already assigned by the websocket-backed host session
- host should treat its websocket-connected student list as the source of truth for which students count toward readiness
- toolbar summary should distinguish at least `ready`, `loading`, `blocked`, `waiting`, and `override-used`
...
- student-level readiness/error state should also be available to the host's student panel, not just the summarized toolbar indicator
...
- a late-joining student should receive the latest active state immediately and should not force a new coordinated-autoplay wait cycle for playback already in progress
...
- host student panel should surface per-student video readiness/error state using the same session student IDs used for readiness aggregation
- host toolbar and student panel should ignore unknown future video capability/state fields rather than failing closed
...
- when any iframe sends `youtubePlayerStatus`, update the per-student readiness map and recompute an aggregate status for the instructor toolbar
...
- when a newly joined student is promoted to `student`, host should immediately send the latest active playback state and current policy state without waiting for another instructor interaction
...
- when active authority changes, host should immediately reapply that instructor's current student-audio and student-local-control policy to students
...
- host student panel should show per-student readiness/error state such as `ready`, `loading`, or `blocked`
...
- schema version bump guidance for a backward-compatible point release of the extended protocol
- new command names
- payload examples
...
- explicit guidance that hosts/iframes ignore unrecognized future video-specific message types and fields
...
- [ ] Specify that existing websocket-backed student session IDs are reused for readiness/error tracking in the host UI.
- [ ] Specify schema versioning and forward-compatibility behavior for unknown video-specific messages/fields.
...
- [ ] Emit enough player-status metadata for the host to populate per-student readiness/error state in the student panel.
...
- [ ] Ensure authority takeover reapplies the new instructor's current student-audio and local-control policy immediately.
...
- [ ] Validate newly joined students receive the latest active playback and policy state immediately.
...
- [ ] Validate last-instructor-wins takeover also reapplies the new authority's audio/local-control policy to connected students.
- [ ] Validate blocked/loading student state appears correctly in the host student panel and toolbar summary.
...
- The host student panel can show each connected student's video readiness/error state using the existing session student IDs.
...
- A student disconnects from the websocket-backed session and should no longer count toward coordinated autoplay readiness.
...
- Two instructors are connected; the last one to take authority should also reapply that instructor's current student-audio and local-control policy immediately.
...
- Readiness signals may flap during load or reconnect:
  - Mitigation: aggregate per-student player status with timestamps and display stable summary states in the toolbar rather than reacting to a single transient event.
...
- Disconnect/readiness mismatches could block autoplay incorrectly:
  - Mitigation: base readiness aggregation on the host websocket-connected student list and drop disconnected students immediately.
...
- Students cannot independently control synchronized playback outside explicitly released regions.
...
- The host can surface per-student readiness/error state in its student panel using existing session student IDs.
...
- Persisted instructor-local student-audio preference is treated as a host toolbar default for future sessions, not as live authority by itself.
- v1 supports one synchronized YouTube player per slide.
- The video-sync message extension should start schema versioning with a backward-compatible point release, and hosts/iframes should ignore unknown future video-specific messages/fields.
