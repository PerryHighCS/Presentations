...
- **Concept:** a small set of labeled SVG nodes arranged on a circle; students drag from one node to another to add or remove an edge (a slightly bowed quadratic-bezier curve rather than a straight chord, so overlapping connections stay visually distinguishable). A stats bar tracks connection count and whole-graph connectivity. Two optional tools escalate with the lesson's guidelines: "Test a Cut" lets a student click an edge to simulate severing it and immediately flags any node that becomes unreachable in red; "Find a Path" lets a student tap two nodes to highlight the shortest remaining route between them and reports the hop count.
- **Mechanism:** node positions are computed from a labels array on a circle (`cx + r*cos/sin`), not hardcoded, so the node count is parametrizable. Dragging is implemented with plain `pointerdown`/`pointermove`/`pointerup` listeners on the SVG root — deliberately *not* `setPointerCapture` — because capturing the pointer caused Chromium to occasionally fire a spurious `pointercancel` when a new drag started from the node that had just been the previous drag's drop target; listening on `window` for move/up sidesteps that capture-lifecycle quirk. A single pointer gesture does double duty: dragging fully from node A to node B commits the edge directly, while a plain tap (down+up on the same node with no distinct second node) arms it as a "selected" node so a following tap on a second node connects them too — giving touch/keyboard users a click-click fallback without a separate code path. Connectivity and shortest-path both run a small BFS over an adjacency map rebuilt from the current edge set (respecting simulated cuts when asked to). Each of the widget's DOM lookups goes through a `getLiveElementById` helper (`querySelectorAll('#id')` filtered to exclude anything under `#storyboard`) since SyncDeck's storyboard strip clones slide markup including duplicate ids, so a plain `document.getElementById` risks returning the inert clone. **Bug worth remembering:** conditionally toggling a class via `classList.toggle(name, someExpression)` is only safe if `someExpression` is coerced to a real boolean first (`!!(...)`) — passing a value that evaluates to `undefined` (e.g. an `&&` chain that starts with a falsy-but-not-boolean guard) makes `classList.toggle` perform a genuine toggle instead of forcing the state, which desyncs the visual state from the data model on every re-render.
- **Reuse:** the node-on-a-circle + drag-to-connect/disconnect mechanism generalizes to any "build a graph" activity (network topology, state-machine wiring, relationship mapping); the cut-simulation and shortest-path tools generalize to any lesson that wants students to reason about redundancy or path length over a graph they built themselves, not a pre-made one.
